...
# Window Class
class Window:
    def __init__(self, title="New GraphMap", size=(400, 400), fullscreen=False, background=(60, 60, 60), frame_cap=80):
        # Arguments
        self._title = title
        self._size = size
        ...
        # Window
        self._window = tk.Canvas(self._root, width=self._size[0], height=self._size[1], highlightthickness=0,
    ...
    # Ticks a frame, used as a unit of time
    def tick(self):
...
class GraphicsPrimitive:
    ...
    def is_drawn(self):
        if self._item in self._window.object_list:
            return True
        else:
            return False
...
class Button:
    ...
    def is_drawn(self):
        if (self,) in self._window.object_list:
            return True
        else:
            return False
